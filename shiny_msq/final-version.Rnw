...
For this shiny application we will use only part of the complete data set that entomologists are more confident in and they think are the most relevant data to analyse. Then we will use  yearly mosquito counts for 8 sites and 20 years of information. However complete data has more sites and years on weakly basis but the information with this desegregation is not comparable.
...
The specie behavior have changed across the years? Are some differences for each specie between sites?  
\item \textit{Between species}: the objective is to analyse more than one specie at the same time. We want to answer: which species are more relevant in each site along all the years? 
...
Our shiny app have 5 tabset panels each of them try to answer one of the previous questions. We will describe how to use and read each tabset information base on an example 
...
\subsection{Tabset 1: Individual species description}
...
\subsection{Second tabset: Spatial comparison} 
