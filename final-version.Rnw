...
\begin{figure}
<<rtab2, dependson=c('data'), echo=FALSE,fig.height=5>>==
d <- subset(msq.geno, geno %in% c('Aedes','Culex') & site %in% c('Cfall', 'Gvalley') )
qplot(data=d,x=year,y=prop.geno)+geom_point()+geom_line(aes(color=site))+facet_grid(facets=~geno)
@
...
p + geom_path(data=ia.s, aes(x=long, y=lat, group=group) ) + facet_wrap(~specie) + scale_fill_gradient2( low='black', high='red') 
dev.off()
@
